...
public class JavaSource implements Serializable, JavaClassParent {
  ...
  private static final String extractType(String typeName) {
    int dotIdx;
    int start = 0;
    int end = typeName.length();

    while (!Character.isUpperCase(typeName.charAt(start))) {
      dotIdx = typeName.indexOf('.', start);
      if (dotIdx > 0)
        start = dotIdx + 1;
      else
        break;
    ...
    if ((dotIdx = typeName.indexOf('.', start)) > 0) end = dotIdx;
